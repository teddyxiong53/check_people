...
function verify(input, key, method, type, signature) {
  if(type === "hmac") {
    ...
    return crypto__default['default'].createVerify(method)
                 .update(input)
...
function sign(input, key, method, type) {
  var base64str;
  if(type === "hmac") {
    base64str = crypto__default['default'].createHmac(method, key).update(input).digest('base64');
  }
  else if(type == "sign") {
    base64str = crypto__default['default'].createSign(method).update(input).sign(key, 'base64');
  }
  else {
    throw new Error('Algorithm type not recognized');
  }
...
const passSecret = "1234";
const tokenExp = 30*24*3600*1000;
...
function encryptPassword(password) {
	const hmac = crypto__default['default'].createHmac('sha1', passSecret$1.toString('ascii'));
...
async function signUp(event) {
	...
	let tokenSecret = crypto__default['default'].randomBytes(16).toString('hex');
	let token = jwtSimple.encode(userInfo, tokenSecret);
